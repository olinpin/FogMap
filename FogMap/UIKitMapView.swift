...
class UIKitMapView: UIViewController, MKMapViewDelegate {
    ...
    private var userCircle: MKCircle?
    ...
    private func observeUserLocation() {
        userLocationCancellable = LocationManager.shared.$userLocation
            ...
            .sink { [weak self] location in
                ...
                if let existing = self.userCircle {
                    self.mapView.removeOverlay(existing)
                }
                let newCircle = MKCircle(center: location.coordinate, radius: 1000)
    ...
    func setInitialLocation() {
        var initialLocation =
            LocationManager.shared.locations.map({ $0.getCLLocationCoordinate2D() }).last ?? nil
        ...
        let region = MKCoordinateRegion(
            center: initialLocation ?? defaultLocation, latitudinalMeters: 10000,
            longitudinalMeters: 10000)
    ...
    private func addCircles() {
        ...
        for coordinate in LocationManager.shared.locations.map({ $0.getCLLocationCoordinate2D() }) {
            if coords.contains(where: {
                coordinate.latitude == $0.latitude && coordinate.longitude == $0.longitude
            }) {
                continue
            }
    ...
    private func updateMask() {
        ...
        for circle in allCircles {
            ...
            let circlePath = UIBezierPath(
                arcCenter: circlePoint, radius: radiusInPoints, startAngle: 0,
